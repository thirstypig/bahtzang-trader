...
<!-- now-tldr -->
An AI trading experiment — Claude makes the buy / sell / hold calls, a small web app handles the data and execution, and a paper-trading account at Alpaca is the live target (no real money yet). **2026-07-22:** Test 5 (id=6) reached 31 trades but FAILED the Phase G zero-losing-weeks gate — and its dashboard was lying: daily snapshots valued any position with a missing price at $0 (Alpha Vantage returns HTTP 200 with a rate-limit notice), reporting **-40.8%** when the portfolio was actually **-7.5%**. Fixed snapshots to price from Alpaca with bounded carry-forward and refuse-to-write-when-unpriceable; backfilled the corrupted history (PR #38). Diagnosis showed the real problem is exit discipline: the 5% stop-loss was never enforced (prompt text only), so positions exited at -10% to -24%, a -$58/trade expectancy. Started the risk engine to fix it (PRD-002): ATR-based broker-held stops + risk-based sizing, core math + broker OTO order shipped & tested, executor wiring next; target is a fresh Test 6. Also built an internal `/docs` knowledge-base system (frontmatter convention, generated living docs, comment inbox) and retrofitted 31 legacy docs. Test 5 preserved like Test 4; no portfolio is currently trading the new rules.
<!-- /now-tldr -->
...
- APScheduler (cron at 10:00 AM ET Mon-Fri + 3:30 PM exit-only risk check)
...
Exit-only cycle (3:30 PM ET daily): sells/holds only — buys suppressed in the executor for ALL decision modes; Claude prompt gets an EXIT-ONLY block with the stop-loss threshold
Virtual positions carry real cost basis + unrealized P&L (average-cost method over executed trades) — compute_virtual_cost_basis()
...
frontend/
  src/
    app/              # Next.js App Router pages
      ...
      testing/        # /testing (test inventory, execution cadence, 556 tests)
      audit-log/      # /audit-log
...
<!-- DOCS:STATUS:START -->
<!-- Generated by npm run docs:refresh on 2026-07-23. Do not edit by hand. -->
...
- `TD-001` Bracket-order support in broker layer (stop_price leg)
- `TD-002` Wire risk sizing + ATR stop into executor entry path
- `TD-003` Add risk_pct / atr_multiple to Portfolio; stop_price to Trade
