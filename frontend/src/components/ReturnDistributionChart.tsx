    ...
    <div className="bz-glass p-5">
      ...
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={bins}>
          ...
          />
          <ReferenceLine x={bins.findIndex((b) => b.midpoint >= 0) >= 0 ? bins[bins.findIndex((b) => b.midpoint >= 0)]?.range : undefined} stroke="rgb(var(--border-strong) / 0.35)" strokeDasharray="3 3" />
          <Bar dataKey="count" radius={[2, 2, 0, 0]}>
            {bins.map((bin, i) => (
              <Cell
                key={i}
                fill={bin.midpoint >= 0 ? "rgb(var(--pos))" : "rgb(var(--neg))"}
                fillOpacity={0.7}
              />
            ))}
          </Bar>
